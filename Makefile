...
install-cli-tools: sanity-check ## Install system packages
	yes|sudo pacman -S --noconfirm --needed base-devel bison bzip2 ca-certificates cloc cmake dos2unix expect ffmpeg foremost gdb gnupg htop bottom hwinfo icu inotify-tools iproute2 jq llvm lsof ltrace make mlocate mplayer ncurses net-tools ngrep nmap openssh openssl parallel perl-image-exiftool pkgconf python-virtualenv re2c readline ripgrep rlwrap socat gnu-netcat sqlite sshpass tmate tor traceroute trash-cli tree unzip vbindiff xclip xz yay zip veracrypt git-delta viu xsv asciinema htmlq neovim glow jless websocat superfile gron exa fastfetch bat sysstat cronie libinput-gestures httpie arp-scan
...
install-gui: sanity-check ## Install gui, i3, polybar, kitty, rofi, picom
	...
	ln -sf /opt/skillarch/config/i3/swipe_desktop.sh ~/.config/i3/swipe_desktop.sh
...
install-hardening: sanity-check ## Install hardening tools
	yes|sudo pacman -S --noconfirm --needed opensnitch arpwatch
	# Install psad through perl script install.pl (root needed)
